...
| R3 | No PII into the model, none out of it | partial | Retrieved tickets and the question pass through `ApplyGuardrail` before the model, and the Converse Guardrail masks the output. A tester saw an unmasked name in a displayed source (`ND280`, 2026-09-22) before the input mask existed. Live run of the two-sided mask pending. Assumes `ApplyGuardrail` returns one output per content block, unverified. |
...
| PII | Bedrock Guardrail, PII set to `ANONYMIZE`. `ApplyGuardrail` masks the question and the retrieved tickets before the Converse call, and the Converse Guardrail masks the output. The index holds the tickets as written. | The assurance is that no PII goes into the model or comes out of it. The Knowledge Base stays faithful to the source, so masking sits on the query path, both sides of the model. The page shows the masked tickets, not the indexed ones. |
...
| Output format | Converse `outputConfig` with a JSON schema (`label` enum, `notes`, `reply`) on Haiku. Sonnet 5 gets the same schema in the prompt and a parser fallback. Parsed with `json.loads`, no Pydantic. | Bedrock lists structured outputs as not supported for Claude Sonnet 5, only for Haiku 4.5; Sonnet rejects `outputConfig` with `output_config.format: Extra inputs are not permitted`. Tool use was the alternative and was rejected: the Guardrail does not evaluate `toolUse.input`, so the reply would bypass PII masking. |
